        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            <div className="space-y-3">
                <div className="aspect-[4/3] w-full rounded-lg bg-blue-50 flex items-center justify-center overflow-hidden">
                    <Image src={product.images[0]} alt={product.name} width={400} height={300} className="h-full w-full object-contain mix-blend-multiply" />
                </div>
                <div className="grid grid-cols-4 gap-2">
                    {product.images.map((src) => (
                        <div key={src} className="aspect-square rounded bg-blue-50 overflow-hidden">
                            <Image src={src} alt="thumb" width={100} height={100} className="h-full w-full object-contain" />
                        </div>
                    ))}
                </div>
            </div>
